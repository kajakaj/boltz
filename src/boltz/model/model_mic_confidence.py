...
class Boltz1(LightningModule):
    def __init__(  # noqa: PLR0915, C901, PLR0912
        ...
        self.change_activations = change_activations
    ...
    def training_step(self, batch: dict[str, Tensor], batch_idx: int) -> Tensor:
        ...
        loss_per_sample = (
            # Average over the batch dimension
            self.training_args.confidence_loss_weight * confidence_loss_dict["loss"]
            + self.training_args.diffusion_loss_weight * diffusion_loss_dict["loss"] / len(batch)
            # globally averaged loss
            + self.training_args.distogram_loss_weight * disto_loss
        )
        self.loss_over_epoch.append(loss_per_sample)
        return loss
